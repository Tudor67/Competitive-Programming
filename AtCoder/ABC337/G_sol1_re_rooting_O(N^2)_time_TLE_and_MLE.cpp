...
void dfs1(int parent, int node, const Graph& G, vector<long long>& f,
          vector<vector<int>>& subtreeValues, vector<int>& lp, vector<int>& lr){
    for(int nextNode: G[node]){
        if(nextNode != parent){
            dfs1(node, nextNode, G, f, subtreeValues, lp, lr);
            f[node] += f[nextNode];
            subtreeValues[node].insert(subtreeValues[node].end(),
                                       subtreeValues[nextNode].begin(), subtreeValues[nextNode].end());
            subtreeValues[nextNode].clear();
        }
    }
    subtreeValues[node].push_back(node);
    for(int val: subtreeValues[node]){
        lp[node] += (int)(val < parent);
        lr[node] += (int)(val < node);
    }
    f[node] += lr[node];
}
...
int main(){
    ...
    vector<vector<int>> subtreeValues(N + 1);
    vector<int> lp(N + 1);
    vector<int> lr(N + 1);
