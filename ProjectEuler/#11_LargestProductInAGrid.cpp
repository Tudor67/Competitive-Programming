...
int main(){
    ...
    for(int i = 0; i < grid.size(); ++i){
        for(int j = 0; j < grid.size(); ++j){
            for(const vector<pair<int, int>> DIRECTION_VECTOR: DIRECTIONS){
                int prod = 1;
                for(const pair<int, int> DIRECTION: DIRECTION_VECTOR){
                    int new_i = i + DIRECTION.first;
                    int new_j = j + DIRECTION.second;
                    if(new_i >= 0 && new_i < grid.size() &&
                       new_j >= 0 && new_j < grid.size()){
                        prod *= grid[new_i][new_j];
                    }else{
                        prod = 0;
                        break;
                    }
