...
private:
    ...
    void buildTree(int node, int l, int r, int startPos, const vector<int>& NEXT_INDEX){
        if(l == r){
            tree[startPos + l] = NEXT_INDEX[r];
        }else{
            int mid = (l + r) / 2;
            buildTree(2 * node + 1, l, mid, startPos + N, NEXT_INDEX);
            buildTree(2 * node + 2, mid + 1, r, startPos + N, NEXT_INDEX);
            merge(tree.begin() + startPos + N + l, tree.begin() + startPos + N + mid + 1,
                  tree.begin() + startPos + N + mid + 1, tree.begin() + startPos + N + r + 1,
                  tree.begin() + startPos + l);
        }
    }

    int countDistinctValues(int node, int l, int r, int startPos, const int L, const int R){
        if(r < L || R < l){
            return 0;
        }
        if(L <= l && r <= R){
            return tree.begin() + startPos + r + 1 -
                   upper_bound(tree.begin() + startPos + l, tree.begin() + startPos + r + 1, R);
        }
        int mid = (l + r) / 2;
        return countDistinctValues(2 * node + 1, l, mid, startPos + N, L, R) +
               countDistinctValues(2 * node + 2, mid + 1, r, startPos + N, L, R);
    }
...
public:
    MergeSortTree(const vector<int>& NEXT_INDEX): N(NEXT_INDEX.size()){
        ...
        tree.resize(levels * minLeaves);
        buildTree(0, 0, N - 1, 0, NEXT_INDEX);
    }
    ...
    int countDistinctValues(const int L, const int R){
        ...
        return countDistinctValues(0, 0, N - 1, 0, L, R);
    }
};
