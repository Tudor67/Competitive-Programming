...
class Solution{
public:
    pair<int, int> longestRepeatingSubstringIdxLen(const string& S){
        ...
        int maxIdx = max_element(lcp.begin(), lcp.end()) - lcp.begin();
        int idx = suff[maxIdx];
        int len = lcp[maxIdx];
...
int main(){
    ...
    pair<int, int> p = Solution().longestRepeatingSubstringIdxLen(s);
    int idx = p.first;
    int len = p.second;
