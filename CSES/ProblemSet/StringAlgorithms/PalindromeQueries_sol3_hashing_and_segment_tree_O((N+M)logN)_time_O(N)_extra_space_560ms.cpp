...
private:
    ...
    void update(int node, int l, int r, const int& POS, const int& VAL){
        if(l == r){
            tree[node] = COEF[r] * (VAL - FIRST_ELEM + 1);
        }else{
            int mid = (l + r) / 2;
            if(POS <= mid){
    ...
    long long query(int node, int l, int r, const int& L, const int& R){
        if(r < L || R < l){
            ...
            int mid = (l + r) / 2;
            long long res = query(2 * node + 1, l, mid, L, R) +
...
public:
    void processPalindromeQueries(const string& S, vector<tuple<int, int, int>>& queries){
        ...
        incCoef[0] = 1;
        for(int i = 1; i < N; ++i){
            incCoef[i] = incCoef[i - 1] * BASE;
            mod(incCoef[i], MODULO);
        }
