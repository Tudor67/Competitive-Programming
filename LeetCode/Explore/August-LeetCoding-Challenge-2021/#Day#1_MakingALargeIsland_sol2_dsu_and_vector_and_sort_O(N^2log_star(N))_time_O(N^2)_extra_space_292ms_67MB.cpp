                ...
                if(grid[row][col] == 0){
                    ...
                    for(const pair<int, int>& DIRECTION: DIRECTIONS){
                        ...
                        if(0 <= neighRow && neighRow < N && 0 <= neighCol && neighCol < N && grid[neighRow][neighCol] == 1){
                            neighGroups.push_back(dsu.find(getIdx(neighRow, neighCol, N)));
                        }
                    }
                    int neighGroupsTotalSize = 1;
                    if(neighGroups.size() >= 1){
                        sort(neighGroups.begin(), neighGroups.end());
                        for(int i = 0; i < (int)neighGroups.size(); ++i){
                            if(i == 0 || neighGroups[i - 1] != neighGroups[i]){
                                neighGroupsTotalSize += groupSize[neighGroups[i]];
                            }
                        }
                    }
