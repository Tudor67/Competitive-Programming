...
public:
    int minCost(vector<int>& houses, vector<vector<int>>& cost, int m, int n, int target) {
        ...
        // At ith iteration:
        //    dp[i % 2][g][c]: min cost to paint houses[0 .. i] with g continuous groups of the same color
        //                     such that houses[i] is painted with color c
        vector<vector<vector<int>>> dp(2, vector<vector<int>>(TARGET_GROUPS + 2, vector<int>(MAX_COLOR + 1, INF)));
        ...
        for(int i = 0; i + 1 < HOUSES; ++i){
            for(int groups = 0; groups <= min(i + 2, TARGET_GROUPS); ++groups){
                for(int color = 0; color <= MAX_COLOR; ++color){
                    dp[(i + 1) % 2][groups][color] = INF;
                }
            }
            for(int groups = 1; groups <= min(i + 1, TARGET_GROUPS); ++groups){
                ...
                for(int color = firstColor; color <= lastColor; ++color){
                    ...
                    for(int nextColor = firstNextColor; nextColor <= lastNextColor; ++nextColor){
                        ...
                        dp[(i + 1) % 2][nextGroups][nextColor] = min(dp[(i + 1) % 2][nextGroups][nextColor],
                                                                     dp[i % 2][groups][color] + nextColorCost);
                    }
                }
            }
        }
        
        int res = *min_element(dp[(HOUSES - 1) % 2][TARGET_GROUPS].begin(), dp[(HOUSES - 1) % 2][TARGET_GROUPS].end());
        if(res == INF){
            res = -1;
        }
