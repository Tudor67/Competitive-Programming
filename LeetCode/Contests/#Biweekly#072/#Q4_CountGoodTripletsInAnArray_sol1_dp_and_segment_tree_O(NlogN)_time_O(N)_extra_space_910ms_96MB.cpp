...
public:
    long long goodTriplets(vector<int>& nums1, vector<int>& nums2) {
        ...
        // dp2[i]: number of pairs (prevPos, i) in v[0 .. i]
        //         such that (prevPos < i) and (v[prevPos] < v[i])
        vector<long long> dp2(N);
        SegmentTree segmentTree(N);
        for(int i = 0; i < N; ++i){
            dp2[i] = segmentTree.query(0, v[i] - 1);
            segmentTree.add(v[i], 1);
        }
        
        // dp3[i]: number of triplets (prevPos0, prevPos1, i) in v[0 .. i]
        //         such that (prevPos0 < prevPos1 < i) and (v[prevPos0] < v[prevPos1] < v[i])
        vector<long long> dp3(N);
        segmentTree.reset();
        for(int i = 0; i < N; ++i){
            dp3[i] = segmentTree.query(0, v[i] - 1);
            segmentTree.add(v[i], dp2[i]);
        }
