class Solution {
private:
    int compare(int i, int j, int len, vector<vector<int>>& c){
        int k = floor(log2(len));
        pair<int, int> iC = {c[k][i], c[k][i + len - (1 << k)]};
        pair<int, int> jC = {c[k][j], c[k][j + len - (1 << k)]};
        if(iC < jC){
            return -1;
        }else if(iC == jC){
            return 0;
    ...
    int numberOfCombinations(string num) {        
        ...
        for(int i = 1; i < N; ++i){
            ...
            for(int j = i; j < N; ++j){
                ...
                dp[i][j] %= MODULO;
                int len = j - i + 1;
                if(k >= 0 && i - k == len && compare(k, i, len, c) <= 0){
                    dp[i][j] += dp[k][i - 1];
                    dp[i][j] %= MODULO;
                }
                k -= 1;
