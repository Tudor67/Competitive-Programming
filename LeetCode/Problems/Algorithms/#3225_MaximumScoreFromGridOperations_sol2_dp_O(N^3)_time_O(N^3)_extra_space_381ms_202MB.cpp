...
public:
    long long maximumScore(vector<vector<int>>& grid) {
        ...
        vector<long long> dpDiffMax(N + 1);

        for(int i = 0; i + 1 < N; ++i){
            fill(dpMax.begin(), dpMax.end(), 0);
            fill(dpDiffMax.begin(), dpDiffMax.end(), 0);
            for(int r2 = 0; r2 <= N; ++r2){
                for(int r1 = 0; r1 <= N; ++r1){
                    dpMax[r1] = max(dpMax[r1], dp[i][r2][r1]);
                    dpDiffMax[r1] = max(dpDiffMax[r1], dp[i][r2][r1] - getColSum(suffSumOfCol[i], r1, r2 - 1));
                }
            }
            ...
            for(int r1 = 0; r1 <= N; ++r1){
                for(int r0 = 0; r0 <= N; ++r0){
                    if(r1 < r0){
                        maximize(dp[i + 1][r1][r0], dpDiffMax[r1] + getColSum(suffSumOfCol[i], r1, r0 - 1));
                    }else{
                        maximize(dp[i + 1][r1][r0], dpMax[r1] + getColSum(suffSumOfCol[i + 1], r0, r1 - 1));
                    }
                }
            }
        }
