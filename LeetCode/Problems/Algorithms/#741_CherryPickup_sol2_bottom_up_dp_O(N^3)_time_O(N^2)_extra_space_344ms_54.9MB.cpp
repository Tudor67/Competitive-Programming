...
public:
    int cherryPickup(vector<vector<int>>& grid) {
        ...
        // dp[i][d1][d2]: max cherries after i (compressed) steps if robot1 is at d1th cell and robot2 is at the d2th cell
        
        dp[0][0][0] = grid[0][0];
        for(int step = 1; step <= 2 * N - 2; ++step){
            for(int d1 = 0; d1 < N; ++d1){
                for(int d2 = 0; d2 < N; ++d2){
                    dp[step % 2][d1][d2] = INT_MIN;
                    ...
                    if(0 <= col1 && col1 < N && 0 <= col2 && col2 < N && grid[row1][col1] != -1 && grid[row2][col2] != -1){
                        ...
                        for(const pair<int, int>& P: vector<pair<int, int>>{{-1, -1}, {-1, 0}, {0, -1}, {0, 0}}){
                            ...
                            if(0 <= prevD1 && 0 <= prevD2){
                                dp[step % 2][d1][d2] = max(cherries + dp[(step - 1) % 2][prevD1][prevD2], dp[step % 2][d1][d2]);
                            }
                        }
                    }
                }
