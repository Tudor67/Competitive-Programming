...
public:
    vector<vector<int>> modifiedGraphEdges(int N, vector<vector<int>>& edges, int source, int destination, int target) {
        ...
        for(int maxSpecialEdges = 1; maxSpecialEdges <= (int)specialEdges.size(); ++maxSpecialEdges){
            setWeightsOfSpecialEdges(weight, specialEdges, maxSpecialEdges);
            int shortestDist = dijkstra(source, destination, weight);
            if(shortestDist <= target){
                int diff = target - shortestDist;
                int a = specialEdges[maxSpecialEdges - 1].first;
                int b = specialEdges[maxSpecialEdges - 1].second;
                weight[a][b] += diff;
                weight[b][a] += diff;
                return extractEdges(edges, weight);
            }
