...
private:
    ...
    bool containsOnlyValidBlockTimes(vector<vector<int>>& timeMasks, vector<vector<int>>& blockTime){
        for(int row = 0; row < 8; ++row){
            for(int col = 0; col < 8; ++col){
                if(blockTime[row][col] != INF){
                    if(getMaxTime(timeMasks[row][col]) != blockTime[row][col]){
                        return false;
                    }
                }
            }
    ...
    void dfs(int level, const int& N, const vector<vector<int>>& POSITIONS, const vector<vector<pair<int, int>>>& DIRECTIONS,
        ...
        if(level == N){
            if(containsOnlyValidBlockTimes(timeMasks, blockTime)){
            ...
            for(const pair<int, int>& DIRECTION: DIRECTIONS[level]){
                ...
                for(int t = 1; t < 8; ++t){
                    ...
                    if(isInsideBoard(nextRow, nextCol)){
                        if(!isVisitedTime(t, timeMasks[nextRow][nextCol])){
                            timeMasks[nextRow][nextCol] += (1 << t);
                            lastUpdatedT = t;
                            if(blockTime[nextRow][nextCol] == INF){
