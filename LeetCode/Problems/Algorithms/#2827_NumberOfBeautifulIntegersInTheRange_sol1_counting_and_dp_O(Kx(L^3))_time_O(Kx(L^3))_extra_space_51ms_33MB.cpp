...
private:
    int countBeautifulIntegers(const int LIMIT, const int K){
        ...
        // dp0[len][rem][c0][c1]: number of integers (with possible leading zeros)
        //                        with length len, remainder rem and c0 even digits and c1 odd digits
        // dp1[len][rem][c0][c1]: number of integers (without leading zeros)
        //                        with length len, remainder rem and c0 even digits and c1 odd digits
        vector<vector<vector<vector<int>>>> dp0(LEN, vector<vector<vector<int>>>(K,
                                                vector<vector<int>>(LEN, vector<int>(LEN))));
        vector<vector<vector<vector<int>>>> dp1(LEN, vector<vector<vector<int>>>(K,
                                                vector<vector<int>>(LEN, vector<int>(LEN))));
        dp0[0][0][0][0] = 1;
        dp0[1][0][1][0] = 1;
        for(int digit = 1; digit <= 9; ++digit){
            dp0[1][digit % K][(int)(digit % 2 == 0)][(int)(digit % 2 == 1)] += 1;
            dp1[1][digit % K][(int)(digit % 2 == 0)][(int)(digit % 2 == 1)] += 1;
        }
            ...
            for(int rem = 0; rem < K; ++rem){
                for(int c0 = 0; c0 <= len; ++c0){
                    ...
                    for(int newDigit = 0; newDigit <= 9; ++newDigit){
                        ...
                        dp0[nextLen][nextRem][nextC0][nextC1] += dp0[len][rem][c0][c1];
                        dp1[nextLen][nextRem][nextC0][nextC1] += dp1[len][rem][c0][c1];
                    }
                }
            }
        }
        ...
        for(int i = 0; i < LEN; ++i){
            ...
            for(int newDigit = startDigit; newDigit < LIMIT_STR[i] - '0'; ++newDigit){
                ...
                if(targetC0 >= 0 && targetC1 >= 0){
                    res += dp0[targetLen][targetRem][targetC0][targetC1];
                }
            }
