...
private:
    ...
    struct TupleHash{
        size_t operator()(const tuple<int, int, int>& T) const {
            size_t row = get<0>(T);
            size_t col = get<1>(T);
            size_t prevCols = get<2>(T);
            return (row << 12) | (col << 9) | (prevCols);
        }
    };
    ...
    int solve(int row, int col, int prevColsMask, vector<vector<char>>& seats, unordered_map<tuple<int, int, int>, int, TupleHash>& memo){
        ...
        tuple<int, int, int> state = {row, col, prevColsMask};
        if(memo.count(state)){
            return memo[state];
        }
        int maxPersons = 0;
