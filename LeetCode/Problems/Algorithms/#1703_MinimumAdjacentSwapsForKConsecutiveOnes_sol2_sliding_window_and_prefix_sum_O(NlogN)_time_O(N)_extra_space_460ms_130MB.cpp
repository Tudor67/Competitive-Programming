...
private:
    ...
    int computeRangeMinMoves(vector<int>& nums, int l, int r, int k,
        ...
        const int MID_ONE_POS = lower_bound(prefOnes.begin(), prefOnes.end(), prefOnes[l] + k / 2) - prefOnes.begin();
...
public:
    int minMoves(vector<int>& nums, int k) {
        ...
        for(int j = 0; j < N; ++j){
            ...
            if(ones == k && nums[j] == 1){
                answer = min(answer, computeRangeMinMoves(nums, i, j, k, prefZeros, prefOnes, prefMoves, suffMoves));
            }
            if(answer == 0){
                break;
            }
