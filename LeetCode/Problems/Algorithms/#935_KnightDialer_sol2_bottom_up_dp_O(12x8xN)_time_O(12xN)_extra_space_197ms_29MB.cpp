...
public:
    int knightDialer(int n) {
        // dp[row][col][n]: number of paths of length n ending at cell (row, col)
                    ...
                    if(!(row == 3 && (col == 0 || col == 2))){
                        for(const pair<int, int>& DIRECTION: DIRECTIONS){
                            ...
                            if(0 <= prevRow && prevRow < 4 && 0 <= prevCol && prevCol < 3){
                                dp[row][col][len] += dp[prevRow][prevCol][len - 1];
                                dp[row][col][len] %= MODULO;
                            }
                        }
                    }
                }
