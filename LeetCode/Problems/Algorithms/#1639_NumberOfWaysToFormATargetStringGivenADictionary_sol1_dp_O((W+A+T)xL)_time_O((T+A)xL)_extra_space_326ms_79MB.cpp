...
public:
    int numWays(vector<string>& words, string target) {
        ...
        // dp[i][j]: number of ways to form target[0 .. i]
        //           when we use chars from words[..][0 .. j]
        vector<vector<long long>> dp(TARGET_LEN, vector<long long>(WORD_LEN));
        for(int j = 0; j < WORD_LEN; ++j){
            if(j == 0){
        ...
        for(int i = 1; i < TARGET_LEN; ++i){
            for(int j = 1; j < WORD_LEN; ++j){
                dp[i][j] = (dp[i][j - 1] + dp[i - 1][j - 1] * count[target[i] - 'a'][j]) % MODULO;
            }
        }

        return dp[TARGET_LEN - 1][WORD_LEN - 1];
    }
};