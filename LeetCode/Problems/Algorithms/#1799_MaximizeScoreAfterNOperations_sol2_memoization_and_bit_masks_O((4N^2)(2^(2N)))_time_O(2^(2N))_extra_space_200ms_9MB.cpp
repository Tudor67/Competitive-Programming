class Solution {
private:
    int computeMaxScore(int step, int maskVisNums, vector<int>& nums, const int& N, vector<int>& memo){
        if(step == 0){
            return 0;
        }
        if(memo[maskVisNums] != -1){
            return memo[maskVisNums];
        }
        int maxScore = 0;
        for(int i = 0; i < 2 * N; ++i){
