...
public:
    int minimumDistance(string word) {
        ...
        for(int i = N - 1; i >= 0; --i){
            ...
            for(int a = 0; a < A; ++a){
                for(int b = 0; b < A; ++b){
                    minimize(dp[i % 2][currCharIdx][b], dp[(i + 1) % 2][a][b] + dist[currCharIdx][a]);
                    minimize(dp[i % 2][a][currCharIdx], dp[(i + 1) % 2][a][b] + dist[currCharIdx][b]);
                }
            }
        }
