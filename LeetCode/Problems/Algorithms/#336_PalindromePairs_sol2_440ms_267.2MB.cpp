...
class Solution {
private:
    bool is_palindrome(const string& word, const int& start_pos, const int& end_pos){
    ...
    void save_all_palindrome_prefixes(TrieNode* node, string& st,
        ...
        if(node != NULL){
            if(node->is_terminal_node && !st.empty() && is_palindrome(st, 0, (int)st.length() - 1)){
    ...
    void add_palindrome_pairs(const string& word, const int& word_id,
        ...
        if(node->is_terminal_node && is_palindrome(word, 0, (int)word.length() - 1)){
            update_answer(word_id, node->word_ids, answer);
        }
        ...
        for(int i = 0; i < word.length(); ++i){
            ...
            node = node->children[edge_id];
            if(node->is_terminal_node && is_palindrome(word, i + 1, (int)word.length() - 1)){
                update_answer(word_id, node->word_ids, answer);
            }
        }
