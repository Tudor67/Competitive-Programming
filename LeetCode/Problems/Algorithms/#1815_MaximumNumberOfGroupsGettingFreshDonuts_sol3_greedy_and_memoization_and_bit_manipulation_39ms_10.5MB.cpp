...
public:
    int maxHappyGroups(int batchSize, vector<int>& groups) {
        ...
        if(batchSize == 1){
            ...
            // greedy
            int fixedGroups = count[0];
            count[0] = 0;
            for(int g = 1; g <= batchSize / 2; ++g){
                if(g + g == batchSize){
                    fixedGroups += count[g] / 2;
                    count[g] %= 2;
                }else{
                    int minCount = min(count[g], count[batchSize - g]);
                    fixedGroups += minCount;
                    count[g] -= minCount;
                    count[batchSize - g] -= minCount;
                }
            }
