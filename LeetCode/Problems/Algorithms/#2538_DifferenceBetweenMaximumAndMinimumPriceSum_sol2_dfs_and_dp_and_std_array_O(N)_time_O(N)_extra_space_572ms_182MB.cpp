...
private:
    ...
    void updateFirstAndSecondMax(const pair<long long, int>& P, array<pair<long long, int>, 2>& s){
        if(P > s[0]){
            s[1] = s[0];
            s[0] = P;
        }else if(P > s[1]){
    ...
    void dfs(int parentNode, int node, const Graph& G, vector<int>& price,
             vector<array<pair<long long, int>, 2>>& s0, vector<array<pair<long long, int>, 2>>& s1){
        bool isLeaf = true;
...
public:
    long long maxOutput(int N, vector<vector<int>>& edges, vector<int>& price) {        
        ...
        //           when node is the root of the tree
        vector<array<pair<long long, int>, 2>> s0(N, {pair<long long, int>{0, -1}, pair<long long, int>{0, -1}});
        vector<array<pair<long long, int>, 2>> s1(N, {pair<long long, int>{0, -1}, pair<long long, int>{0, -1}});
