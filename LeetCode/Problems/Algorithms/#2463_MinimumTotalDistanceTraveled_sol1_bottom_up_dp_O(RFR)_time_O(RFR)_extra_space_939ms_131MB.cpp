...
public:
    long long minimumTotalDistance(vector<int>& r, vector<vector<int>>& f) {
        ...
        // dp[i][j][k]: min cost (== total distance)
        //              to repair sorted robots r[0 .. i] at sorted factories f[0 .. j]
        //              such that (the remaining limit of f[j]) >= k
        vector<vector<vector<long long>>> dp(R, vector<vector<long long>>(F, vector<long long>(R + 1, INF)));
        for(int i = 0; i < R; ++i){
            for(int j = 0; j < F; ++j){
                // repair r[i] at f[0 .. j - 1]
                dp[i][j][f[j][1]] = min(dp[i][j][f[j][1]], getValue(dp, i, j - 1, 0));
                
                // repair r[i] at f[j]
                for(int k = f[j][1] - 1; k >= 0; --k){
                    dp[i][j][k] = min({dp[i][j][k],
                                       dp[i][j][k + 1],
                                       getValue(dp, i - 1, j, k + 1) + getDistance(r[i], f[j][0])});
                }
            }
        }
