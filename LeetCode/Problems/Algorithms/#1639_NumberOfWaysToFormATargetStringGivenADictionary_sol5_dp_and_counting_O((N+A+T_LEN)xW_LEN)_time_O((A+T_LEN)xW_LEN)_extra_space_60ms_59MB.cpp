...
public:
    int numWays(vector<string>& words, string target) {
        ...
        // dp[i][j]: number of ways to form target[0 .. i] from the first (j + 1) chars of words
        vector<vector<int>> dp(T_LEN, vector<int>(W_LEN));
        ...
        for(int i = 1; i < T_LEN; ++i){
            for(int j = 1; j < W_LEN; ++j){
                dp[i][j] = (dp[i][j - 1] +
                            dp[i - 1][j - 1] * 1LL * freq[target[i] - 'a'][j]) % MODULO;
            }
        }

        return dp[T_LEN - 1][W_LEN - 1];
    }
};