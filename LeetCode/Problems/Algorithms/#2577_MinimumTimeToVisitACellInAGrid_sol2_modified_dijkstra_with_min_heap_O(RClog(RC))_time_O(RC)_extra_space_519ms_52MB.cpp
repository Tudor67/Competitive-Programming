...
public:
    int minimumTime(vector<vector<int>>& grid) {
        ...
        priority_queue<TIII, vector<TIII>, greater<TIII>> statesMinHeap;
        minTime[0][0] = 0;
        statesMinHeap.push({minTime[0][0], 0, 0});

        while(!statesMinHeap.empty()){
            int currentTime = get<0>(statesMinHeap.top());
            int row = get<1>(statesMinHeap.top());
            int col = get<2>(statesMinHeap.top());
            statesMinHeap.pop();
            ...
            for(const pair<int, int>& DIRECTION: DIRECTIONS){
                ...
                if(0 <= nextRow && nextRow < ROWS && 0 <= nextCol && nextCol < COLS){
                    ...
                    if(nextValidTime < minTime[nextRow][nextCol]){
                        ...
                        statesMinHeap.push({minTime[nextRow][nextCol], nextRow, nextCol});
                    }
                }
            }
        }
