...
public:
    int maxValue(vector<vector<int>>& events, int k) {
        ...
        vector<tuple<int, int, int>> timeTypeIdx(2 * N);
        for(int j = 0; j < N; ++j){
            timeTypeIdx[j] = {events[j][0], EVENT_START, j};
            timeTypeIdx[j + N]= {events[j][1], EVENT_END, j};
        }
        
        sort(timeTypeIdx.begin(), timeTypeIdx.end());
        
        // closestLeftEventIdx[j] = maximum k, such that events[k][1] < events[j][0]
        vector<int> closestLeftEventIdx(N);
        int prevIdx = 0;
        for(const tuple<int, int, int>& T: timeTypeIdx){
            int eventTime, eventType, eventIdx;
            tie(eventTime, eventType, eventIdx) = T;
            if(eventType == EVENT_START){
                closestLeftEventIdx[eventIdx] = prevIdx;
            }else{
                prevIdx = eventIdx;
            }
        }
