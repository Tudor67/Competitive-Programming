...
public:
    int findPaths(int ROWS, int COLS, int maxMove, int startRow, int startCol) {
        ...
        // At kth iteration:
        //    dp[k % 2][row][col]: number of paths from (startRow + 1, startCol + 1) to (row, col)
                ...
                for(int col = 0; col <= COLS + 1; ++col){
                    ...
                    for(const pair<int, int>& DIRECTION: DIRECTIONS){
                        ...
                        if(1 <= neighRow && neighRow <= ROWS && 1 <= neighCol && neighCol <= COLS){
                            dp[moves % 2][row][col] += dp[(moves - 1) % 2][neighRow][neighCol];
                            dp[moves % 2][row][col] %= MODULO;
                        }
                    }
                    if(row == 0 || row == ROWS + 1 || col == 0 || col == COLS + 1){
                        res += dp[moves % 2][row][col];
                        res %= MODULO;
                    }
                }
            }
