...
private:
    ...
    void computeSubtreeRepr(Trie* node, unordered_map<Trie*, size_t>& subtreeRepr){
        ...
        subtreeRepr[node] = STR_HASH_OBJ(childrenTreeStr);
    }
    
    void collectUniqueFolders(Trie* node, vector<string>& path, unordered_map<Trie*, size_t>& subtreeRepr,
                              unordered_map<size_t, int>& reprCount, vector<vector<string>>& uniqueFolders){
        for(const pair<string, Trie*>& P: node->children){
            const string& CHILD_STR = P.first;
            Trie* child = P.second;
...
public:
    ...
    void computeSubtreeRepr(unordered_map<Trie*, size_t>& subtreeRepr){
        subtreeRepr.clear();
        computeSubtreeRepr(this, subtreeRepr);
    }
    
    void collectUniqueFolders(unordered_map<Trie*, size_t>& subtreeRepr,
                              unordered_map<size_t, int>& reprCount, vector<vector<string>>& uniqueFolders){
        vector<string> path;
...
public:
    vector<vector<string>> deleteDuplicateFolder(vector<vector<string>>& paths) {
        ...
        unordered_map<Trie*, size_t> subtreeRepr;
        trie.computeSubtreeRepr(subtreeRepr);
        
        unordered_map<size_t, int> reprCount;
        for(const pair<Trie*, size_t>& P: subtreeRepr){
            size_t repr = P.second;
            reprCount[repr] += 1;
        }
