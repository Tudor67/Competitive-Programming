...
public:
    int minimumDifference(vector<int>& nums) {
        ...
        for(int mask = 0; mask <= FULL_MASK; ++mask){
            ...
            sums[bitsCount].push_back(sum);
        }
        
        for(int k = 0; k <= N; ++k){
            sort(sums[k].begin(), sums[k].end());
            sums[k].resize(unique(sums[k].begin(), sums[k].end()) - sums[k].begin());
        }
        ...
        for(int mask = 0; mask <= FULL_MASK && minDiff > 0; ++mask){
            ...
            auto it = lower_bound(sums[N - bitsCount].begin(), sums[N - bitsCount].end(), TOTAL_SUM / 2 - sum);
