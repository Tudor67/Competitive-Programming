class Solution {
private:
    int countLessOrEqual(vector<int>& nums, int limit){
        vector<unordered_map<int, int>> count(2);
        for(int num: nums){
            count[0][num] += 1;
        }
        
        // --- Stage 1: count all pairs (i, j) such that nums[i] ^ nums[j] == limit ---
        // for each fixed (num, numCount):
        //     - find (complementNum, complementNumCount) such that num ^ numComplement == limit;
        //     - update pairsCount:
        //              pairsCount += (numCount * (numCount - 1)) if num == numComplement;
        //              pairsCount += (numCount * complementNumCount) if num != numComplement;
        int pairsCount = 0;
        for(const pair<int, int>& P: count[0]){
            int num = P.first;
            int numCount = P.second;
            int complementNum = num ^ limit;
            if(count[0].count(complementNum)){
                if(num == complementNum){
                    pairsCount += numCount * (numCount - 1);
                }else{
                    pairsCount += numCount * count[0][complementNum];
                }
            }
        }
        
        // --- Stage 2: count all pairs (i, j) such that nums[i] ^ nums[j] < limit ---
        // At current step:
        //    for each fixed (num, numCount):
