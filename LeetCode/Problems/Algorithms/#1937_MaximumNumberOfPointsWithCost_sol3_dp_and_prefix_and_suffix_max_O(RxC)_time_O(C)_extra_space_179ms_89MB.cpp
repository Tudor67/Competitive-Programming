...
public:
    long long maxPoints(vector<vector<int>>& points) {
        ...
        for(int row = 1; row < ROWS; ++row){
            ...
            long long prefMax = 0;
            for(int col = 0; col < COLS; ++col){
                prefMax = max(prefMax, maxPoints[(row - 1) % 2][col] + col);
                maxPoints[row % 2][col] = max(maxPoints[row % 2][col], prefMax - col + points[row][col]);
            }

            long long suffMax = 0;
            for(int col = COLS - 1; col >= 0; --col){
                suffMax = max(suffMax, maxPoints[(row - 1) % 2][col] + COLS - 1 - col);
                maxPoints[row % 2][col] = max(maxPoints[row % 2][col], points[row][col] + suffMax - (COLS - 1 - col));
            }
        }
