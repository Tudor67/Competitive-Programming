class Solution {
private:
    int compareSubstrings(int i1, int i2, int len, vector<vector<int>>& c){
...
public:
    string shortestBeautifulSubstring(string s, int k) {
        ...
        for(int j = 1; j <= LOG_N; ++j){
            ...
            for(int i = 0; i < N; ++i){
                int l = c[j - 1][i];
                int r = (i + len / 2 < N ? c[j - 1][i + len / 2] : -1);
                v[i].first = {l, r};
                v[i].second = i;
            }
        ...
        for(int r = 0; r < N; ++r){
            ...
            if(ones == k){
                int len = r - l + 1;
                if(minLen > len || (minLen == len && compareSubstrings(startIdx, l, len, c) > 0)){
                    minLen = len;
                    startIdx = l;
                }
            }
