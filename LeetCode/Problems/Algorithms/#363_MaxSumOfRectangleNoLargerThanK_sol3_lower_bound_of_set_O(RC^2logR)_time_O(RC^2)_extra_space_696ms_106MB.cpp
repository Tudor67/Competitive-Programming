        ...
        for(int row = 0; row < ROWS; ++row){
            for(int col1 = 0; col1 < COLS; ++col1){
                for(int col2 = col1; col2 < COLS; ++col2){
                    ...
                    if(it != colDiffs[col1][col2].end() && it->first - prevSum <= k){
                        answer = max(it->first - prevSum, answer);
                    }
                    if(it != colDiffs[col1][col2].begin() && prev(it)->first - prevSum <= k){
                        answer = max(prev(it)->first - prevSum, answer);
                    }
