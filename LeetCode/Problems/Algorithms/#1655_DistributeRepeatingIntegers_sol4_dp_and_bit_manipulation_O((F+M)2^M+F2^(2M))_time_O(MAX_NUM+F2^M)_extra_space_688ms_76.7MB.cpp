...
public:
    bool canDistribute(vector<int>& nums, vector<int>& quantity) {
        ...
        for(int fIdx = 0; !isPossible[FULL_MASK] && fIdx < F; ++fIdx){
            for(int prevMask = FULL_MASK; prevMask >= 0; --prevMask){
                if(isPossible[prevMask]){
                    for(int mask: masksFor[fIdx]){
                        isPossible[prevMask | mask] = true;
                    }
                }
            }
        }
