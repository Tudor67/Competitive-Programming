    ...
    int minimumDistance(string word) {
        ...
        for(int i = N - 1; i >= 0; --i){
            ...
            for(int ffc = 0; ffc < A; ++ffc){
                ...
                minimize(dp[i % 2][ffc], dist[currCharIdx][nextCharIdx] + dp[(i + 1) % 2][ffc]);

                // option 2: use the free finger
                //           free finger becomes the previous active finger (at nextCharIdx)
                minimize(dp[i % 2][nextCharIdx], dist[currCharIdx][ffc] + dp[(i + 1) % 2][ffc]);
            }
        }
