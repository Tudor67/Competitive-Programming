...
private:
    ...
    bool isValid(const int& MID_POS, const vector<long long>& MODULOS,
        ...
        for(int i = 0; i < (int)MODULOS.size(); ++i){
            // normalized lHash for ss[0 .. MID_POS] with increasing BASE powers
            long long lHash = hInc[i][MID_POS];
            // normalized rHash for ss[MID_POS .. 2 * MID_POS] with decreasing BASE powers
            long long rHash = hDec[i][2 * MID_POS];
            long long rem = hDec[i][MID_POS - 1] * p[i][MID_POS + 1];
            mod(rem, MODULOS[i]);
            rHash -= rem;
            mod(rHash, MODULOS[i]);
            // distinct hashes => ss[0 .. MID_POS] != reverse(ss[MID_POS .. 2 * MID_POS])
            if(lHash != rHash){
                return false;
            }
        }
