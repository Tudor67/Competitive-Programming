            ...
            for(int prefMax = 1; prefMax <= M; ++prefMax){
                for(int i = 0; i + 1 < N; ++i){
                    for(int newNum = 1; newNum <= prefMax; ++newNum){
                        dp[searchCost][prefMax][i + 1] += dp[searchCost][prefMax][i];
            ...
            for(int prefMax = 1; prefMax <= M; ++prefMax){
                for(int i = 0; i + 1 < N; ++i){
                    for(int newNum = prefMax + 1; newNum <= M; ++newNum){
                        dp[searchCost + 1][newNum][i + 1] += dp[searchCost][prefMax][i];
                        dp[searchCost + 1][newNum][i + 1] %= MODULO;
                    }
                }
            }
        }
