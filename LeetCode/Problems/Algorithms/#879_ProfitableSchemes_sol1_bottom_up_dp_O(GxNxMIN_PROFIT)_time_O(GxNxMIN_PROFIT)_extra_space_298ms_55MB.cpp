...
public:
    int profitableSchemes(int N, int minProfit, vector<int>& group, vector<int>& profit) {
        ...
        // dp[groupIdx][members][prof]: number of different subsets with profit prof
        //                              and total number of participants equal to members
        //                              when we use only groups from group[0 .. groupIdx]
        vector<vector<vector<int>>> dp(G, vector<vector<int>>(N + 2, vector<int>(minProfit + 2)));
        ...
        for(int groupIdx = 0; groupIdx + 1 < G; ++groupIdx){
            ...
            for(int members = 0; members <= N + 1; ++members){
                for(int prof = 0; prof <= minProfit + 1; ++prof){
                    ...
                    dp[nextGroupIdx][nextMembers][nextProf] += dp[groupIdx][members][prof];
                    dp[nextGroupIdx][nextMembers][nextProf] %= MODULO;
                }
            }
        }
        ...
        for(int members = 0; members <= N; ++members){
            res += dp[G - 1][members][minProfit];
            res %= MODULO;
            res += dp[G - 1][members][minProfit + 1];
            res %= MODULO;
        }
