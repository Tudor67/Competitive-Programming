class Solution {
private:
    int computeMaxScore(int studentIdx, int maskFreeMentors, const int& M, vector<vector<int>>& compatibilityScore, vector<vector<int>>& memo){
        if(maskFreeMentors == 0 || studentIdx < 0 || M <= studentIdx){
            return 0;
        }
        if(memo[studentIdx][maskFreeMentors] != -1){
            return memo[studentIdx][maskFreeMentors];
        }
        int maxScore = 0;
        ...
        memo[studentIdx][maskFreeMentors] = maxScore;
        return maxScore;
    }
...
public:
    int maxCompatibilitySum(vector<vector<int>>& students, vector<vector<int>>& mentors) {
        ...
        vector<vector<int>> memo(M, vector<int>(FULL_MASK + 1, -1));
